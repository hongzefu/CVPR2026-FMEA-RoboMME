...
% For instance, a household robot may be asked to retrieve a coffee cup stored earlier in a cupboard, return books to their original positions on the shelf, or fold the laundry after a  human demonstration.
% \todo{my worry about the fetch plate task is that it doesnt necessarily need memory in the sense that the paper is arguing for, people might be triggered to think this is a scene graph paper.}
% \todo{my suggestion (NF) would be to just use examples from your teaser figure rather than mention these tasks., That would need the teaser to move up to the first page, is that okay for ICML?}
