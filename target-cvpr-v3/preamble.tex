...
\newcommand{\wrapttlabel}[1]{\centering\arraybackslash\ttfamily\seqsplit{#1}}
\newcommand{\wrapbflabel}[1]{\centering\arraybackslash\bfseries\seqsplit{#1}}
...
%% --- Tighter float / caption spacing to reclaim space on the 8-page body ---
%% (titlesec was tried and reverted — it leaks the spacing length into
%%  the rendered heading text when combined with cvpr.sty's section overrides.)
...
% \captionsetup[sub]{aboveskip=2pt,belowskip=0pt}
%% Disable column-bottom justification so reclaimed float space stays at the
%% bottom of short columns instead of being injected as a giant inter-paragraph
%% gap (this is what produced the visible hole on page 2 left column).
%\raggedbottom
